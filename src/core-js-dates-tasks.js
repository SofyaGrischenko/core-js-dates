...
function getNextFriday(date) {
  ...
  const daysUntilFriday = (5 - curDay + 7) % 7 || 7;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(elapsedTime / 86400000) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  const newYear = new Date(Date.UTC(newDate.getUTCFullYear(), 0, 1));

  return Math.ceil(((newDate - newYear) / 86400000 + 1) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formDate(currentDate));
